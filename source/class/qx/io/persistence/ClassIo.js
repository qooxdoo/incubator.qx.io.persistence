    ...
    _fromJsonValue(ctlr, value, propertyDef, propertyPath) {
      ...
      const convertArray = arr => {
        ...
        for (let i = 0; i < arr.length; i++) {
          if (arr[i]) {
            let arrayType = null;
            if (typeof arr[i]._classname == "string") {
              arrayType = qx.Class.getByName(arr[i]._classname);
              if (!arrayType) {
                this.error(`Unable to convert IObject in _fromJsonValue because cannot find class ${arr[i]._classname}`);
              }
            }
